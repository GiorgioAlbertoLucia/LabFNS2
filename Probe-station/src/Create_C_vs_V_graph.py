...
if __name__=='__main__':
    ...
    legend = TLegend(0.3, 0.6, 0.86, 0.83)
    legend.AddEntry(gLGAD,'LGAD, -1 * V, f = 5kHz, rev. pol.','p')
    legend.AddEntry(gPin,'PiN, -1 * V, f = 10 kHz,  rev. pol.','p')
    legend.AddEntry(gStrip,'Strip, f = 1 kHz, dir. pol.','p')
    legend.Draw("same")
    canvas.Modified()
    canvas.Update()
