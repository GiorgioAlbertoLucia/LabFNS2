...
if __name__=='__main__':
    ...
    legend3.AddEntry(gLGAD,'LGAD, #font[122]{-}1 * V, f = 5kHz, rev. pol.','p')
    legend3.SetTextSize(0.045)
    legend3.Draw("same")
    
    canvas.cd(1)
    hFrame = canvas.cd(1).DrawFrame(-10,0,230,655,"C-V curves of Hamamatsu Strip ; Reverse Bias Voltage  (V); Capacitance (pF)")
    gStrip.Draw("same,p")
    legend1 = TLegend(0.25, 0.5, 0.55, 0.83)
    legend1.AddEntry(gStrip,'Strip, f = 1 kHz, rev. pol.','p')
    ...
    legend2.AddEntry(gPin,'PiN, #font[122]{-}1 * V, f = 10 kHz,  rev. pol.','p')
